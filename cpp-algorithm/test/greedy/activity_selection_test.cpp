...
TEST(ActivitySelectionTest, RecursiveActivitySelection)
{
    ...
    std::vector<int> result = activity_selection::recursive_activity_selector(
...
TEST(ActivitySelectionTest, GreedyActivitySelection)
{
    ...
    const std::vector<int> result = activity_selection::greedy_activity_selector(start_times, end_times);
    EXPECT_EQ(result, expected_result);
}
