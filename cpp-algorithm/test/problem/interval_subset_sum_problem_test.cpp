...
TEST(IntervalSubsetSumTest, Simple)
{
    ...
    const auto result = problem::SimpleSubsetSum(seq);
...
TEST(IntervalSubsetSumTest, Simple2)
{
    ...
    const auto result = problem::SimpleSubsetSum2(seq);
    ASSERT_EQ(result, expected_value);
}
...
TEST(IntervalSubsetSumTest, DivideAndConquer)
{
    ...
    const auto result = problem::DivideAndConquerSubsetSum(seq, 0, static_cast<int>(seq.size() - 1));
    ASSERT_EQ(result, expected_value);
}
...
TEST(IntervalSubsetSumTest, DynamicProgramming)
{
    ...
    const auto result = problem::DynamicProgrammingSubsetSum(seq);
    ASSERT_EQ(result, expected_value);
}
