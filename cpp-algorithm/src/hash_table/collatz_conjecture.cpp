...
auto CollatzConjecture::GenerateCollatzSequence(const long long number, std::unordered_map<long long, std::vector<long long>>& hash_table) -> std::vector<long long>
{
    if (number <= 0)
    {
        return {};
...
auto CollatzConjecture::FindNumbersSatisfyingCollatzConjecture(const long long number) -> std::vector<long long>
{
    ...
    for (auto i = 1LL; i < number; i++)
    {
        ...
        if (auto sequence = GenerateCollatzSequence(i, hash_table);
            sequence.back() == 1)
        {
