...
namespace BTree
{
    ...
    public:
         ...
         */
        std::pair<Node*, const int> Search(Node* node, char key);
    };
}
...
inline void BTree::Tree::SplitChild(Node* node, int index)
{
    ...
    std::vector<char> right_keys =
        std::vector<char>(left_child->keys.begin() + degree, left_child->keys.end());
    ...
    if (left_child->is_leaf == false)
    {
        std::vector<Node*> right_children =
            std::vector<Node*>(left_child->children.begin() + degree, left_child->children.end());
...
// ----------------------------------------------------------------------------
inline std::pair<BTree::Node*, const int> BTree::Tree::Search(Node* node, char key)
{
    int index = 0;
    int key_count = static_cast<int>(node->keys.size());
