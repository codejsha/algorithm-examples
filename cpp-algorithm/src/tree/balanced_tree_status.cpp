...
auto BalancedTree::IsBalanced(const BinarySearchTree::Node<int>* tree) -> bool
{
    return CheckBalanced(tree).is_balanced;
}

auto BalancedTree::CheckBalanced(const BinarySearchTree::Node<int>* tree) -> BalancedStatus
{
    if (tree == nullptr)
    {
        return {true, -1};
