...
std::tuple<gsl_matrix_int*, gsl_matrix_char*> common_subsequence::longest_common_subsequence_length(
...
void common_subsequence::longest_common_subsequence(const std::string& seq1, const gsl_matrix_char* direction_matrix,
    const int length1, const int length2, std::string& result)
{
    ...
    {
        longest_common_subsequence(seq1, direction_matrix, length1 - 1, length2 - 1, result);
        result = result + seq1[length1 - 1];
    }
    else if (gsl_matrix_char_get(direction_matrix, length1, length2) == '|')
    {
        longest_common_subsequence(seq1, direction_matrix, length1 - 1, length2, result);
    }
    else
    {
        longest_common_subsequence(seq1, direction_matrix, length1, length2 - 1, result);
    }
}

void common_subsequence::print_optimal_matrix(const std::string& seq1, const std::string& seq2,
