...
- Rabin-Karp algorithm: [c++](https://github.com/codejsha/algorithm-examples/tree/main/cpp-algorithm/src/string)([test](https://github.com/codejsha/algorithm-examples/tree/main/cpp-algorithm/test/string)) | Use the hash function to find all occurrences of a pattern in a string. This has $\theta(\text{pattern\_size})$ preprocessing time and $\theta(\text{text\_size} - \text{pattern\_size} + 1) \text{pattern\_size}$ time complexity.
